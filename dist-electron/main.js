...
function getPowershell() {
  ...
  if (_windows$h) {
    ...
    if (fs$t.existsSync(defaultPath)) {
...
function getWmic() {
  if (os$8.type() === "Windows_NT" && !wmicPath) {
    ...
    if (!fs$t.existsSync(wmicPath)) {
      try {
        const wmicPathArray = execSync$b("WHERE WMIC", execOptsWin)
          .toString()
          .split("\r\n");
...
function smartMonToolsInstalled() {
  ...
  if (_windows$h) {
    try {
      const pathArray = execSync$b("WHERE smartctl 2>nul", execOptsWin)
        .toString()
        .split("\r\n");
  ...
  if (_linux$g || _darwin$g || _freebsd$f || _openbsd$f || _netbsd$f) {
    try {
      const pathArray = execSync$b("which smartctl 2>/dev/null", execOptsLinux)
        .toString()
        .split("\r\n");
...
function darwinXcodeExists() {
  const cmdLineToolsExists = fs$t.existsSync(
    "/Library/Developer/CommandLineTools/usr/bin/",
  );
  const xcodeAppExists = fs$t.existsSync(
    "/Applications/Xcode.app/Contents/Developer/Tools",
  );
  const xcodeExists = fs$t.existsSync("/Library/Developer/Xcode/");
...
function time() {
  ...
  if (_darwin$f || _linux$f) {
    try {
      const stdout = execSync$a(
        "date +%Z && date +%z && ls -l /etc/localtime 2>/dev/null",
        util$j.execOptsLinux,
      );
...
function getLogoFile(distro) {
  ...
  if (_windows$g) {
    ...
    distro.indexOf("mac os") !== -1 ||
    distro.indexOf("macos") !== -1
  ) {
    result2 = "apple";
    ...
    process.nextTick(() => {
        ...
        exec$f(
          ...
          function (error, stdout) {
            ...
            if (prettyName.indexOf(result2.distro + " ") === 0) {
              releaseVersion = prettyName
                .replace(result2.distro + " ", "")
                .trim();
            }
      ...
      if (_darwin$f) {
        exec$f(
          ...
          function (error, stdout) {
            ...
            result2.release = (
              util$j.getValue(lines, "ProductVersion", ":", true, true) +
              " " +
              util$j.getValue(lines, "ProductVersionExtra", ":", true, true)
            ).trim();
            result2.build = util$j.getValue(lines, "BuildVersion");
            result2.logofile = getLogoFile(result2.distro);
            result2.codename = "macOS";
            result2.codename =
              result2.release.indexOf("10.4") > -1
                ? "OS X Tiger"
                : result2.codename;
            result2.codename =
              result2.release.indexOf("10.5") > -1
                ? "OS X Leopard"
                : result2.codename;
            result2.codename =
              result2.release.indexOf("10.6") > -1
                ? "OS X Snow Leopard"
                : result2.codename;
            result2.codename =
              result2.release.indexOf("10.7") > -1
                ? "OS X Lion"
                : result2.codename;
            result2.codename =
              result2.release.indexOf("10.8") > -1
                ? "OS X Mountain Lion"
                : result2.codename;
            result2.codename =
              result2.release.indexOf("10.9") > -1
                ? "OS X Mavericks"
                : result2.codename;
            result2.codename =
              result2.release.indexOf("10.10") > -1
                ? "OS X Yosemite"
                : result2.codename;
            result2.codename =
              result2.release.indexOf("10.11") > -1
                ? "OS X El Capitan"
                : result2.codename;
            result2.codename =
              result2.release.indexOf("10.12") > -1
                ? "Sierra"
                : result2.codename;
            result2.codename =
              result2.release.indexOf("10.13") > -1
                ? "High Sierra"
                : result2.codename;
            result2.codename =
              result2.release.indexOf("10.14") > -1
                ? "Mojave"
                : result2.codename;
            result2.codename =
              result2.release.indexOf("10.15") > -1
                ? "Catalina"
                : result2.codename;
            result2.codename = result2.release.startsWith("11.")
              ? "Big Sur"
              : result2.codename;
            result2.codename = result2.release.startsWith("12.")
              ? "Monterey"
              : result2.codename;
            result2.codename = result2.release.startsWith("13.")
              ? "Ventura"
              : result2.codename;
            result2.codename = result2.release.startsWith("14.")
              ? "Sonoma"
              : result2.codename;
            result2.codename = result2.release.startsWith("15.")
              ? "Sequoia"
              : result2.codename;
            result2.codename = result2.release.startsWith("16.")
              ? "Tahoe"
              : result2.codename;
            result2.codename = result2.release.startsWith("26.")
              ? "Tahoe"
              : result2.codename;
            result2.uefi = true;
            result2.codepage = util$j.getCodepage();
      ...
      if (_windows$g) {
        ...
        try {
          ...
          util$j.promiseAll(workload).then((data) => {
            ...
            result2.servicepack =
              util$j.getValue(lines, "ServicePackMajorVersion", ":").trim() +
              "." +
              util$j.getValue(lines, "ServicePackMinorVersion", ":").trim();
            result2.codepage = util$j.getCodepage();
...
function isUefiLinux() {
  return new Promise((resolve) => {
    process.nextTick(() => {
      fs$s.stat("/sys/firmware/efi", function (err) {
        if (!err) {
          return resolve(true);
        } else {
          ...
          for (let key in versionObject) {
            if ({}.hasOwnProperty.call(versionObject, key)) {
              if (
                ...
                !{}.hasOwnProperty.call(result2.versions, key)
              ) {
    ...
    process.nextTick(() => {
      ...
      try {
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "homebrew")) {
          ...
          exec$f(`${cmd} --version`, function (error, stdout) {
            if (!error) {
              ...
              appsObj.versions.homebrew = (
                brew.toLowerCase().split(" ")[1] || ""
              ).trim();
            }
            functionProcessed();
          });
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "git")) {
          if (_darwin$f) {
            const gitHomebrewExists =
              fs$s.existsSync("/usr/local/Cellar/git") ||
              fs$s.existsSync("/opt/homebrew/bin/git");
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "postfix")) {
          exec$f("postconf -d | grep mail_version", function (error, stdout) {
            if (!error) {
              ...
              appsObj.versions.postfix = util$j.getValue(
                postfix,
                "mail_version",
                "=",
                true,
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "python")) {
          if (_darwin$f) {
            try {
              ...
              const gitHomebrewExists1 = fs$s.existsSync(
                "/usr/local/Cellar/python",
              );
              const gitHomebrewExists2 = fs$s.existsSync(
                "/opt/homebrew/bin/python",
              );
              if (
                (util$j.darwinXcodeExists() &&
                  util$j.semverCompare("12.0.1", osVersion) < 0) ||
                gitHomebrewExists1 ||
                gitHomebrewExists2
              ) {
                const cmd2 = gitHomebrewExists1
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "python3")) {
          if (_darwin$f) {
            const gitHomebrewExists =
              fs$s.existsSync("/usr/local/Cellar/python3") ||
              fs$s.existsSync("/opt/homebrew/bin/python3");
            if (util$j.darwinXcodeExists() || gitHomebrewExists) {
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "pip")) {
          if (_darwin$f) {
            const gitHomebrewExists =
              fs$s.existsSync("/usr/local/Cellar/pip") ||
              fs$s.existsSync("/opt/homebrew/bin/pip");
            if (util$j.darwinXcodeExists() || gitHomebrewExists) {
        ...
        if ({}.hasOwnProperty.call(appsObj.versions, "pip3")) {
          if (_darwin$f) {
            const gitHomebrewExists =
              fs$s.existsSync("/usr/local/Cellar/pip3") ||
              fs$s.existsSync("/opt/homebrew/bin/pip3");
            if (util$j.darwinXcodeExists() || gitHomebrewExists) {
        ...
        try {
          ...
          util$j
            ...
            .then((stdout) => {
              ...
              if (stdout) {
                if (
                  stdout.toString().toLowerCase().indexOf("powershell") >= 0
                ) {
                  result3 = "PowerShell";
                }
              }
...
echo -n "hardware: "; cat /sys/class/dmi/id/product_uuid 2> /dev/null; echo;`;
      ...
      if (_freebsd$e || _openbsd$e || _netbsd$e) {
        exec$f("sysctl -i kern.hostid kern.hostuuid", function (error, stdout) {
          const lines = stdout.toString().split("\n");
          result2.hardware = util$j
            .getValue(lines, "kern.hostid", ":")
            .toLowerCase();
          result2.os = util$j
    ...
    process.nextTick(() => {
      ...
      if (_linux$e || _freebsd$d || _openbsd$d || _netbsd$d) {
        exec$e(
          ...
          function (error, stdout) {
            let lines = stdout.toString().split("\n");
            result2.manufacturer = cleanDefaults(
              util$i.getValue(lines, "manufacturer"),
            );
            result2.model = cleanDefaults(
              util$i.getValue(lines, "product name"),
            );
            result2.version = cleanDefaults(util$i.getValue(lines, "version"));
            result2.serial = cleanDefaults(
              util$i.getValue(lines, "serial number"),
            );
            result2.uuid = cleanDefaults(
              util$i.getValue(lines, "uuid"),
            ).toLowerCase();
            ...
            try {
              lines = execSync$9(cmd, util$i.execOptsLinux)
                .toString()
                .split("\n");
              result2.manufacturer = cleanDefaults(
                result2.manufacturer === ""
                  ? util$i.getValue(lines, "sys_vendor")
                  : result2.manufacturer,
              );
              result2.model = cleanDefaults(
                result2.model === ""
                  ? util$i.getValue(lines, "product_name")
                  : result2.model,
              );
              result2.version = cleanDefaults(
                result2.version === ""
                  ? util$i.getValue(lines, "product_version")
                  : result2.version,
              );
              result2.serial = cleanDefaults(
                result2.serial === ""
                  ? util$i.getValue(lines, "product_serial")
                  : result2.serial,
              );
              result2.uuid = cleanDefaults(
                result2.uuid === ""
                  ? util$i.getValue(lines, "product_uuid").toLowerCase()
                  : result2.uuid,
              );
            } catch (e) {
              util$i.noop();
            ...
            if (
              result2.model.toLowerCase() === "virtualbox" ||
              result2.model.toLowerCase() === "kvm" ||
              result2.model.toLowerCase() === "virtual machine" ||
              result2.model.toLowerCase() === "bochs" ||
              result2.model.toLowerCase().startsWith("vmware") ||
              result2.model.toLowerCase().startsWith("droplet")
            ) {
            ...
            if (
              result2.manufacturer.toLowerCase().startsWith("vmware") ||
              result2.manufacturer.toLowerCase() === "xen"
            ) {
              result2.virtual = true;
            ...
            if (!result2.virtual) {
              try {
                const disksById = execSync$9(
                  "ls -1 /dev/disk/by-id/ 2>/dev/null; pciconf -lv  2>/dev/null",
                  util$i.execOptsLinux,
                ).toString();
                if (
                  disksById.indexOf("_QEMU_") >= 0 ||
                  disksById.indexOf("QEMU ") >= 0
                ) {
                  result2.virtual = true;
            ...
            if (_freebsd$d || _openbsd$d || _netbsd$d) {
              try {
                ...
                if (!result2.uuid) {
                  result2.uuid = util$i
                    .getValue(lines2, "kern.hostuuid", ":")
                    .toLowerCase();
                }
                if (!result2.serial || result2.serial === "-") {
                  result2.serial = util$i
                    .getValue(lines2, "kern.hostid", ":")
                    .toLowerCase();
                }
                if (!result2.model || result2.model === "Computer") {
                  result2.model = util$i
                    .getValue(lines2, "hw.model", ":")
                    .trim();
                }
              } catch (e) {
                util$i.noop();
            ...
            if (
              !result2.virtual &&
              (os$6.release().toLowerCase().indexOf("microsoft") >= 0 ||
                os$6.release().toLowerCase().endsWith("wsl2"))
            ) {
            ...
            if (
              (_freebsd$d || _openbsd$d || _netbsd$d) &&
              !result2.virtualHost
            ) {
              try {
                const procInfo = execSync$9(
                  "dmidecode -t 4",
                  util$i.execOptsLinux,
                );
                const procLines = procInfo.toString().split("\n");
                const procManufacturer = util$i.getValue(
                  procLines,
                  "manufacturer",
                  ":",
                  true,
            ...
            if (
              fs$r.existsSync("/.dockerenv") ||
              fs$r.existsSync("/.dockerinit")
            ) {
              result2.model = "Docker Container";
            }
            try {
              const stdout2 = execSync$9(
              ...
              if (lines2.length > 0) {
                ...
                result2.virtual = true;
                if (
                  stdout2.toString().toLowerCase().indexOf("vmware") >= 0 &&
                  !result2.virtualHost
                ) {
                  result2.virtualHost = "VMware";
                }
                if (
                  stdout2.toString().toLowerCase().indexOf("qemu") >= 0 &&
                  !result2.virtualHost
                ) {
                  result2.virtualHost = "QEMU";
                }
                if (
                  stdout2.toString().toLowerCase().indexOf("xen") >= 0 &&
                  !result2.virtualHost
                ) {
                  result2.virtualHost = "Xen";
                }
                if (
                  stdout2.toString().toLowerCase().indexOf("kvm") >= 0 &&
                  !result2.virtualHost
                ) {
                  result2.virtualHost = "KVM";
                }
            ...
            if (
              ...
              fs$r.readFile("/proc/cpuinfo", function (error2, stdout2) {
                if (!error2) {
                  let lines2 = stdout2.toString().split("\n");
                  result2.model = util$i
                    .getValue(lines2, "hardware", ":", true)
                    .toUpperCase();
                  result2.version = util$i
                    .getValue(lines2, "revision", ":", true)
                    .toLowerCase();
        ...
        exec$e(
          ...
          function (error, stdout) {
            if (!error) {
              ...
              const model = util$i.getAppleModel(
                util$i.getValue(lines, "model", "=", true),
              );
              result2.manufacturer = util$i.getValue(
                lines,
                "manufacturer",
                "=",
                true,
              ...
              result2.serial = util$i.getValue(
                lines,
              ...
              result2.uuid = util$i
                ...
                .toLowerCase();
              result2.sku =
                util$i.getValue(lines, "board-id", "=", true) ||
                util$i.getValue(lines, "target-sub-type", "=", true);
            }
            if (callback) {
              callback(result2);
            }
                      ...
                      util$i
                        ...
                        .then((stdout3, error3) => {
                          if (!error3) {
                            ...
                            if (
                              lines2.indexOf("VRTUAL") >= 0 ||
                              lines2.indexOf("A M I ") >= 0 ||
                              lines2.indexOf("VirtualBox") >= 0 ||
                              lines2.indexOf("VMWare") >= 0 ||
                              lines2.indexOf("Xen") >= 0 ||
                              lines2.indexOf("Parallels") >= 0
                            ) {
                              result2.virtual = true;
                              if (
                                lines2.indexOf("VirtualBox") >= 0 &&
    ...
    process.nextTick(() => {
      ...
      if (_linux$e || _freebsd$d || _openbsd$d || _netbsd$d) {
        ...
        exec$e(cmd, function (error, stdout) {
          ...
          try {
            lines = execSync$9(cmd2, util$i.execOptsLinux)
              .toString()
              .split("\n");
            result2.vendor = !result2.vendor
              ? util$i.getValue(lines, "bios_vendor")
              : result2.vendor;
            result2.version = !result2.version
              ? util$i.getValue(lines, "bios_version")
              : result2.version;
            datetime = util$i.getValue(lines, "bios_date");
            result2.releaseDate = !result2.releaseDate
              ? util$i.parseDateTime(datetime).date
              : result2.releaseDate;
          } catch (e) {
            util$i.noop();
          }
          ...
          util$i
            ...
            .then((stdout, error) => {
              if (!error) {
                ...
                const version2 = util$i.getValue(
                  lines,
                  "SMBIOSBIOSVersion",
                  ":",
                );
                if (description.indexOf(" Version ") !== -1) {
                  ...
                  result2.vendor = util$i.getValue(lines, "manufacturer", ":");
                  result2.version =
                    version2 || util$i.getValue(lines, "version", ":");
                }
                result2.releaseDate = util$i.getValue(
                  lines,
                  "releasedate",
                  ":",
                );
                ...
                result2.serial = cleanDefaults(
                  util$i.getValue(lines, "serialnumber", ":"),
                );
              }
              if (callback) {
                callback(result2);
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux$e || _freebsd$d || _openbsd$d || _netbsd$d) {
        ...
        util$i.promiseAll(workload).then((data) => {
          let lines = data.results[0]
            ? data.results[0].toString().split("\n")
            : [""];
          result2.manufacturer = cleanDefaults(
          ...
          try {
            ...
            result2.manufacturer = cleanDefaults(
              !result2.manufacturer
                ? util$i.getValue(lines, "board_vendor")
                : result2.manufacturer,
            );
            result2.model = cleanDefaults(
              !result2.model
                ? util$i.getValue(lines, "board_name")
                : result2.model,
            );
            result2.version = cleanDefaults(
              !result2.version
                ? util$i.getValue(lines, "board_version")
                : result2.version,
            );
            result2.serial = cleanDefaults(
              !result2.serial
                ? util$i.getValue(lines, "board_serial")
                : result2.serial,
            );
            result2.assetTag = cleanDefaults(
              !result2.assetTag
                ? util$i.getValue(lines, "board_asset_tag")
                : result2.assetTag,
            );
          } catch (e) {
            util$i.noop();
          ...
          lines = data.results[1]
            ...
            : [""];
          result2.memMax =
            util$i.toInt(util$i.getValue(lines, "Maximum Capacity")) *
              1024 *
              1024 *
              1024 || null;
          result2.memSlots =
            util$i.toInt(util$i.getValue(lines, "Number Of Devices")) || null;
      ...
      if (_windows$f) {
        try {
          ...
          const maxCapacityAttribute = win10plus
            ? "MaxCapacityEx"
            : "MaxCapacity";
          workload.push(
            util$i.powerShell(
          ...
          util$i.promiseAll(workload).then((data) => {
            ...
            if (!result2.model) {
              result2.model = cleanDefaults(
                util$i.getValue(lines, "product", ":"),
              );
            }
            result2.version = cleanDefaults(
              util$i.getValue(lines, "version", ":"),
            );
            result2.serial = cleanDefaults(
              util$i.getValue(lines, "serialnumber", ":"),
            );
            result2.assetTag = cleanDefaults(
              util$i.getValue(lines, "partnumber", ":"),
            );
            if (!result2.assetTag) {
              result2.assetTag = cleanDefaults(
                util$i.getValue(lines, "sku", ":"),
              );
            }
            lines = data.results[1]
              ? data.results[1].toString().split("\r\n")
              : [""];
            result2.memMax =
              util$i.toInt(util$i.getValue(lines, maxCapacityAttribute, ":")) *
                (win10plus ? 1024 : 1) || null;
            result2.memSlots =
              util$i.toInt(util$i.getValue(lines, "MemoryDevices", ":")) ||
              null;
            if (callback) {
              callback(result2);
            }
    ...
    process.nextTick(() => {
      ...
      if (_linux$e || _freebsd$d || _openbsd$d || _netbsd$d) {
        ...
        exec$e(cmd, function (error, stdout) {
          let lines = stdout.toString().split("\n");
          result2.manufacturer = cleanDefaults(
            util$i.getValue(lines, "chassis_vendor"),
          );
          const ctype = parseInt(
            util$i.getValue(lines, "chassis_type").replace(/\D/g, ""),
          );
          result2.type = cleanDefaults(
            ctype && !isNaN(ctype) && ctype < chassisTypes.length
              ? chassisTypes[ctype - 1]
              : "",
          );
          result2.version = cleanDefaults(
            util$i.getValue(lines, "chassis_version"),
          );
          result2.serial = cleanDefaults(
            util$i.getValue(lines, "chassis_serial"),
          );
          result2.assetTag = cleanDefaults(
            util$i.getValue(lines, "chassis_asset_tag"),
          );
          if (callback) {
            callback(result2);
          }
        ...
        exec$e(
          ...
          function (error, stdout) {
            if (!error) {
              ...
              result2.assetTag =
                util$i.getValue(lines, "board-id", "=", true) ||
                util$i.getValue(lines, "target-type", "=", true);
              result2.sku = util$i.getValue(
                lines,
                "target-sub-type",
                "=",
                true,
          ...
          util$i
            ...
            .then((stdout, error) => {
              if (!error) {
                ...
                result2.model = cleanDefaults(
                  util$i.getValue(lines, "model", ":"),
                );
                const ctype = parseInt(
                  util$i
                    .getValue(lines, "ChassisTypes", ":")
                    .replace(/\D/g, ""),
                );
                result2.type =
                  ctype && !isNaN(ctype) && ctype < chassisTypes.length
                ...
                if (!result2.assetTag) {
                  result2.assetTag = cleanDefaults(
                    util$i.getValue(lines, "SMBIOSAssetTag", ":"),
                  );
                }
      ...
      cpuFlags().then((flags) => {
        ...
        if (_darwin$d) {
          exec$d(
            ...
            function (error, stdout) {
              let lines = stdout.toString().split("\n");
              const modelline = util$h.getValue(
                lines,
              ...
              result2.speedMin = util$h.getValue(lines, "hw.cpufrequency_min")
                ? util$h.getValue(lines, "hw.cpufrequency_min") / 1e9
                : result2.speed;
              result2.speedMax = util$h.getValue(lines, "hw.cpufrequency_max")
                ? util$h.getValue(lines, "hw.cpufrequency_max") / 1e9
                : result2.speed;
              result2.vendor =
                util$h.getValue(lines, "machdep.cpu.vendor") || "Apple";
              result2.family =
                util$h.getValue(lines, "machdep.cpu.family") ||
                util$h.getValue(lines, "hw.cpufamily");
              result2.model = util$h.getValue(lines, "machdep.cpu.model");
              result2.stepping =
                util$h.getValue(lines, "machdep.cpu.stepping") ||
                util$h.getValue(lines, "hw.cpusubfamily");
        ...
        if (_linux$d) {
          ...
          exec$d(
            ...
            function (error, stdout) {
              ...
              modelline =
                util$h.getValue(lines, "bios model name") || modelline;
              modelline = util$h.cleanString(modelline);
              const modellineParts = modelline.split("@");
              result2.brand = modellineParts[0].trim();
              result2.speed = modellineParts[1]
                ? parseFloat(modellineParts[1].trim())
                : 0;
              if (
                result2.speed === 0 &&
                (result2.brand.indexOf("AMD") > -1 ||
                  result2.brand.toLowerCase().indexOf("ryzen") > -1)
              ) {
              ...
              result2.speedMin =
                Math.round(
                  parseFloat(
                    util$h.getValue(lines, "cpu min mhz").replace(/,/g, "."),
                  ) / 10,
                ) / 100;
              result2.speedMax =
                Math.round(
                  parseFloat(
                    util$h.getValue(lines, "cpu max mhz").replace(/,/g, "."),
                  ) / 10,
                ) / 100;
              result2 = cpuBrandManufacturer(result2);
              result2.vendor = cpuManufacturer(
                util$h.getValue(lines, "vendor id"),
              );
              ...
              if (result2.cache.l1d) {
                result2.cache.l1d =
                  parseInt(result2.cache.l1d) *
                  (result2.cache.l1d.indexOf("M") !== -1
                    ? 1024 * 1024
                    : result2.cache.l1d.indexOf("K") !== -1
                      ? 1024
                      : 1);
              }
              ...
              if (result2.cache.l1i) {
                result2.cache.l1i =
                  parseInt(result2.cache.l1i) *
                  (result2.cache.l1i.indexOf("M") !== -1
                    ? 1024 * 1024
                    : result2.cache.l1i.indexOf("K") !== -1
                      ? 1024
                      : 1);
              }
              ...
              if (result2.cache.l2) {
                result2.cache.l2 =
                  parseInt(result2.cache.l2) *
                  (result2.cache.l2.indexOf("M") !== -1
                    ? 1024 * 1024
                    : result2.cache.l2.indexOf("K") !== -1
                      ? 1024
                      : 1);
              }
              ...
              if (result2.cache.l3) {
                result2.cache.l3 =
                  parseInt(result2.cache.l3) *
                  (result2.cache.l3.indexOf("M") !== -1
                    ? 1024 * 1024
                    : result2.cache.l3.indexOf("K") !== -1
                      ? 1024
                      : 1);
              }
              const threadsPerCore =
                util$h.getValue(lines, "thread(s) per core") || "1";
              ...
              const coresPerSocket = parseInt(
                util$h.getValue(lines, "core(s) per socket"),
                10,
              );
              result2.physicalCores = coresPerSocket
                ? coresPerSocket * processorsInt
                : result2.cores / threadsPerCoreInt;
              result2.performanceCores =
                threadsPerCoreInt > 1
                  ? result2.cores - result2.physicalCores
                  : result2.cores;
              result2.efficiencyCores =
                threadsPerCoreInt > 1
                  ? result2.cores - threadsPerCoreInt * result2.performanceCores
                  : 0;
        ...
        if (_freebsd$c || _openbsd$c || _netbsd$c) {
          ...
          exec$d(
            ...
            function (error, stdout) {
              ...
              result2.speed = modelline.split("@")[1]
              ...
              result2.speedMax =
                Math.round(
                  parseFloat(
                    util$h.getValue(lines, "max speed").replace(/Mhz/g, ""),
                  ) / 10,
                ) / 100;
              result2 = cpuBrandManufacturer(result2);
              result2.vendor = cpuManufacturer(
                util$h.getValue(lines, "manufacturer"),
              );
              ...
              for (let i = 0; i < cache.length; i++) {
                lines = cache[i].split("\n");
                let cacheType = util$h
                  .getValue(lines, "Socket Designation")
                  .toLowerCase()
                  .replace(" ", "-")
                  .split("-");
                cacheType = cacheType.length ? cacheType[0] : "";
                const sizeParts = util$h
        ...
        if (_windows$e) {
          try {
            ...
            Promise.all(workload).then((data) => {
              ...
              const countThreads = util$h.getValue(
                lines,
...
function getCpuCurrentSpeedSync() {
  ...
  if (cpus && cpus.length && cpus[0].hasOwnProperty("speed")) {
    for (let i in cpus) {
      speeds.push(
        cpus[i].speed > 100 ? (cpus[i].speed + 1) / 1e3 : cpus[i].speed / 10,
      );
    }
  } else if (_linux$d) {
    try {
      const speedStrings = execSync$8(
        'cat /proc/cpuinfo | grep "cpu MHz" | cut -d " " -f 3',
        util$h.execOptsLinux,
      )
        .toString()
        .split("\n")
    ...
    process.nextTick(() => {
      ...
      if (_linux$d) {
        try {
          ...
          const parts = execSync$8(cmd2, util$h.execOptsLinux)
            .toString()
            .split("-----\n");
          if (parts.length === 2) {
          ...
          exec$d(cmd, function (error, stdout) {
            ...
            lines.forEach((line) => {
              ...
              if (value && label && label.toLowerCase() === "tctl") {
                tctl = result2.main =
                  Math.round(parseInt(value, 10) / 100) / 10;
              }
              if (
                value &&
                (label === void 0 ||
            ...
            if (result2.cores.length > 0) {
              if (result2.main === null) {
                result2.main = Math.round(
                  result2.cores.reduce((a, b) => a + b, 0) /
                    result2.cores.length,
                );
              }
        ...
        exec$d("sysctl dev.cpu | grep temp", function (error, stdout) {
          if (!error) {
            ...
            if (result2.cores.length) {
              result2.main =
                Math.round((sum / result2.cores.length) * 100) / 100;
            }
          }
          if (callback) {
            callback(result2);
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
        ...
        try {
          fs$p.readFile("/proc/meminfo", function (error, stdout) {
            if (!error) {
              ...
              result2.buffcache =
                result2.buffers + result2.cached + result2.slab;
              let available = parseInt(
                util$g.getValue(lines, "memavailable"),
                10,
              );
              result2.available = available
                ? available * 1024
                : result2.free + result2.buffcache;
              result2.active = result2.total - result2.available;
              result2.swaptotal = parseInt(
                util$g.getValue(lines, "swaptotal"),
                10,
              );
              result2.swaptotal = result2.swaptotal
                ? result2.swaptotal * 1024
                : 0;
              result2.swapfree = parseInt(
                util$g.getValue(lines, "swapfree"),
                10,
              );
              result2.swapfree = result2.swapfree ? result2.swapfree * 1024 : 0;
              result2.swapused = result2.swaptotal - result2.swapfree;
              result2.writeback = parseInt(
                util$g.getValue(lines, "writeback"),
                10,
              );
              result2.writeback = result2.writeback
                ? result2.writeback * 1024
                : 0;
              result2.dirty = parseInt(util$g.getValue(lines, "dirty"), 10);
              result2.dirty = result2.dirty ? result2.dirty * 1024 : 0;
              result2.reclaimable = parseInt(
                util$g.getValue(lines, "sreclaimable"),
                10,
              );
              result2.reclaimable = result2.reclaimable
                ? result2.reclaimable * 1024
                : 0;
            }
            if (callback) {
              callback(result2);
            ...
            function (error, stdout) {
              if (!error) {
                ...
                const pagesize = parseInt(
                  util$g.getValue(lines, "vm.stats.vm.v_page_size"),
                  10,
                );
                const inactive =
                  parseInt(
                    util$g.getValue(lines, "vm.stats.vm.v_inactive_count"),
                    10,
                  ) * pagesize;
                const cache =
                  parseInt(
                    util$g.getValue(lines, "vm.stats.vm.v_cache_count"),
                    10,
                  ) * pagesize;
                result2.total = parseInt(
                  util$g.getValue(lines, "hw.realmem"),
                  10,
                );
                if (isNaN(result2.total)) {
                  result2.total = parseInt(
                    util$g.getValue(lines, "hw.physmem"),
                    10,
                  );
                }
                result2.free =
                  parseInt(
                    util$g.getValue(lines, "vm.stats.vm.v_free_count"),
                    10,
                  ) * pagesize;
                result2.buffcache = inactive + cache;
                result2.available = result2.buffcache + result2.free;
                result2.active =
                  result2.total - result2.free - result2.buffcache;
                result2.swaptotal = 0;
                result2.swapfree = 0;
                result2.swapused = 0;
              }
      ...
      if (_darwin$c) {
        let pageSize = 4096;
        try {
          let sysPpageSize = util$g.toInt(
            execSync$7("sysctl -n vm.pagesize").toString(),
          );
          ...
          exec$c(
            ...
            function (error, stdout) {
              if (!error) {
                let lines = stdout.toString().split("\n");
                result2.active =
                  (parseInt(util$g.getValue(lines, "Pages active"), 10) || 0) *
                  pageSize;
                result2.reclaimable =
                  (parseInt(util$g.getValue(lines, "Pages inactive"), 10) ||
                    0) * pageSize;
...
function memLayout(callback) {
  function getManufacturer(manId) {
    ...
    if (
      ...
      {}.hasOwnProperty.call(RAM_manufacturers, manIdSearch)
    ) {
        ...
        exec$c(
          ...
          function (error, stdout) {
            if (!error) {
              ...
              devices.forEach(function (device) {
                ...
                if (parseInt(util$g.getValue(lines, "Size"), 10) > 0) {
                  const totalWidth = util$g.toInt(
                    util$g.getValue(lines, "Total Width"),
                  );
                  const dataWidth = util$g.toInt(
                    util$g.getValue(lines, "Data Width"),
                  );
                  result2.push({
                    size,
                    bank,
                    ...
                    clockSpeed: util$g.getValue(
                      lines,
                      "Configured Clock Speed:",
                    )
                      ? parseInt(
                          util$g.getValue(lines, "Configured Clock Speed:"),
                          10,
                        )
                      : util$g.getValue(lines, "Speed:")
                        ? parseInt(util$g.getValue(lines, "Speed:"), 10)
                        : null,
                    formFactor: util$g.getValue(lines, "Form Factor:"),
                    manufacturer: getManufacturer(
                      util$g.getValue(lines, "Manufacturer:"),
                    ),
                    partNum: util$g.getValue(lines, "Part Number:"),
                    serialNum: util$g.getValue(lines, "Serial Number:"),
                    voltageConfigured:
                      parseFloat(
                        util$g.getValue(lines, "Configured Voltage:"),
                      ) || null,
                    voltageMin:
                      parseFloat(util$g.getValue(lines, "Minimum Voltage:")) ||
                      null,
                    voltageMax:
                      parseFloat(util$g.getValue(lines, "Maximum Voltage:")) ||
                      null,
                  });
                } else {
                  result2.push({
            ...
            if (!result2.length) {
              ...
              try {
                let stdout2 = execSync$7(
                  "cat /proc/cpuinfo 2>/dev/null",
                  util$g.execOptsLinux,
                );
                let lines = stdout2.toString().split("\n");
                let version2 = util$g
                ...
                if (util$g.isRaspberry(lines)) {
                  ...
                  result2[0].type =
                    version2 && version2[2] && version2[2] === "3"
                      ? "LPDDR4"
                      : result2[0].type;
                  result2[0].type =
                    version2 && version2[2] && version2[2] === "4"
                      ? "LPDDR4X"
                      : result2[0].type;
                  result2[0].ecc = false;
                  result2[0].clockSpeed =
                    (version2 && version2[2] && clockSpeed[version2[2]]) || 400;
                  result2[0].clockSpeed =
                    version2 && version2[4] && version2[4] === "d"
                      ? 500
                      : result2[0].clockSpeed;
                  ...
                  let freq =
                    parseInt(
                      util$g.getValue(lines, "sdram_freq", "=", true),
                      10,
                    ) || 0;
                  if (freq) {
                  ...
                  lines = stdout2.toString().split("\n");
                  let voltage =
                    parseFloat(util$g.getValue(lines, "volt", "=", true)) || 0;
                  if (voltage) {
      ...
      if (_darwin$c) {
        exec$c("system_profiler SPMemoryDataType", function (error, stdout) {
          ...
          if (!result2.length) {
            ...
            const manufacturerId = util$g.getValue(
              lines,
              "      Manufacturer:",
            );
            if (size && type) {
            ...
            .then((stdout, error) => {
              if (!error) {
                ...
                devices.forEach(function (device) {
                  let lines = device.split("\r\n");
                  const dataWidth = util$g.toInt(
                    util$g.getValue(lines, "DataWidth", ":"),
                  );
                  const totalWidth = util$g.toInt(
                    util$g.getValue(lines, "TotalWidth", ":"),
                  );
                  const size =
                    parseInt(util$g.getValue(lines, "Capacity", ":"), 10) || 0;
                  ...
                  if (size) {
                    result2.push({
                      size,
                      bank:
                        util$g.getValue(lines, "BankLabel", ":") +
                        (tagInt[1] ? "/" + tagInt[1] : ""),
                      // BankLabel
                      type: memoryTypes[
                        parseInt(
                          util$g.getValue(lines, "MemoryType", ":"),
                          10,
                        ) ||
                          parseInt(
                            util$g.getValue(lines, "SMBIOSMemoryType", ":"),
                            10,
                          )
                      ],
                      ecc:
                      ...
                      clockSpeed:
                        parseInt(
                          util$g.getValue(lines, "ConfiguredClockSpeed", ":"),
                          10,
                        ) ||
                        parseInt(util$g.getValue(lines, "Speed", ":"), 10) ||
                        0,
                      formFactor:
                        FormFactors[
                          parseInt(
                            util$g.getValue(lines, "FormFactor", ":"),
                            10,
                          ) || 0
                        ],
                      manufacturer: getManufacturer(
                      ...
                      voltageConfigured:
                        (parseInt(
                          util$g.getValue(lines, "ConfiguredVoltage", ":"),
                          10,
                        ) || 0) / 1e3,
                      voltageMin:
                        (parseInt(
                          util$g.getValue(lines, "MinVoltage", ":"),
                          10,
                        ) || 0) / 1e3,
                      voltageMax:
                        (parseInt(
                          util$g.getValue(lines, "MaxVoltage", ":"),
                          10,
                        ) || 0) / 1e3,
                    });
                  }
...
function parseWinBatteryPart(lines, designedCapacity, fullChargeCapacity) {
  ...
  if (status >= 0) {
    ...
    result2.maxCapacity =
      fullChargeCapacity ||
      parseInt(util$f.getValue(lines, "DesignCapacity", ":") || 0);
    result2.designedCapacity = parseInt(
      util$f.getValue(lines, "DesignCapacity", ":") || designedCapacity,
    );
    result2.voltage =
      parseInt(util$f.getValue(lines, "DesignVoltage", ":") || 0) / 1e3;
    result2.capacityUnit = "mWh";
    result2.percent = parseInt(
      util$f.getValue(lines, "EstimatedChargeRemaining", ":") || 0,
    );
    result2.currentCapacity = parseInt(
      (result2.maxCapacity * result2.percent) / 100,
    );
    result2.isCharging =
      (statusValue >= 6 && statusValue <= 9) ||
      statusValue === 11 ||
      (statusValue !== 3 && statusValue !== 1 && result2.percent < 100);
...
var battery = function (callback) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux$b) {
        let battery_path = "";
        if (fs$o.existsSync("/sys/class/power_supply/BAT1/uevent")) {
          battery_path = "/sys/class/power_supply/BAT1/";
        } else if (fs$o.existsSync("/sys/class/power_supply/BAT0/uevent")) {
          battery_path = "/sys/class/power_supply/BAT0/";
        }
        let acConnected = false;
        let acPath = "";
        if (fs$o.existsSync("/sys/class/power_supply/AC/online")) {
          acPath = "/sys/class/power_supply/AC/online";
        } else if (fs$o.existsSync("/sys/class/power_supply/AC0/online")) {
          acPath = "/sys/class/power_supply/AC0/online";
        }
        if (acPath) {
          const file2 = fs$o.readFileSync(acPath);
        ...
        if (battery_path) {
          fs$o.readFile(battery_path + "uevent", function (error, stdout) {
            if (!error) {
              ...
              result2.voltage =
                parseInt(
                  "0" + util$f.getValue(lines, "POWER_SUPPLY_VOLTAGE_NOW", "="),
                  10,
                ) / 1e6;
              result2.capacityUnit = result2.voltage ? "mWh" : "mAh";
              result2.cycleCount = parseInt(
                "0" + util$f.getValue(lines, "POWER_SUPPLY_CYCLE_COUNT", "="),
                10,
              );
              result2.maxCapacity = Math.round(
                (parseInt(
                  "0" +
                    util$f.getValue(
                      lines,
              ...
              result2.designedCapacity = Math.round(
                (parseInt(
                  ...
                  1e3) *
                  (desingedMinVoltage || result2.voltage || 1),
              );
              result2.currentCapacity = Math.round(
                (parseInt(
                  "0" + util$f.getValue(lines, "POWER_SUPPLY_CHARGE_NOW", "="),
                  10,
                ) /
                  1e3) *
                  (result2.voltage || 1),
              ...
              if (!result2.maxCapacity) {
                ...
                result2.designedCapacity =
                  (parseInt(
                    ...
                    1e3) |
                  result2.maxCapacity;
                result2.currentCapacity =
                  parseInt(
                    "0" +
                      util$f.getValue(lines, "POWER_SUPPLY_ENERGY_NOW", "="),
                    10,
                  ) / 1e3;
              }
              const percent = util$f.getValue(
                lines,
                "POWER_SUPPLY_CAPACITY",
                "=",
              );
              const energy = parseInt(
                "0" + util$f.getValue(lines, "POWER_SUPPLY_ENERGY_NOW", "="),
                10,
              );
              const power = parseInt(
                "0" + util$f.getValue(lines, "POWER_SUPPLY_POWER_NOW", "="),
                10,
              );
              const current = parseInt(
                "0" + util$f.getValue(lines, "POWER_SUPPLY_CURRENT_NOW", "="),
                10,
              );
              ...
              if (result2.maxCapacity && result2.currentCapacity) {
                ...
                if (!percent) {
                  result2.percent =
                    (100 * result2.currentCapacity) / result2.maxCapacity;
                }
              }
              ...
              if (energy && power) {
                ...
                result2.timeRemaining = Math.floor(
                  (result2.currentCapacity / current) * 60,
                );
              }
              result2.type = util$f.getValue(
                lines,
                "POWER_SUPPLY_TECHNOLOGY",
                "=",
              );
              result2.model = util$f.getValue(
                lines,
                "POWER_SUPPLY_MODEL_NAME",
                "=",
              );
              result2.manufacturer = util$f.getValue(
                lines,
                "POWER_SUPPLY_MANUFACTURER",
                "=",
              );
              result2.serial = util$f.getValue(
                lines,
        ...
        exec$b(
          ...
          function (error, stdout) {
            ...
            const batteries = parseInt(
              "0" + util$f.getValue(lines, "hw.acpi.battery.units"),
              10,
            );
            const percent = parseInt(
              "0" + util$f.getValue(lines, "hw.acpi.battery.life"),
              10,
            );
            result2.hasBattery = batteries > 0;
            result2.cycleCount = null;
            result2.isCharging =
              util$f.getValue(lines, "hw.acpi.acline") !== "1";
          ...
          function (error, stdout) {
            if (stdout) {
              let lines = stdout
                ...
                .split("\n");
              result2.cycleCount = parseInt(
                "0" + util$f.getValue(lines, "cyclecount", "="),
                10,
              );
              result2.voltage =
                parseInt("0" + util$f.getValue(lines, "voltage", "="), 10) /
                1e3;
              result2.capacityUnit = result2.voltage ? "mWh" : "mAh";
              result2.maxCapacity = Math.round(
                parseInt(
                  "0" + util$f.getValue(lines, "applerawmaxcapacity", "="),
                  10,
                ) * (result2.voltage || 1),
              );
              result2.currentCapacity = Math.round(
                parseInt(
                  "0" + util$f.getValue(lines, "applerawcurrentcapacity", "="),
                  10,
                ) * (result2.voltage || 1),
              );
              result2.designedCapacity = Math.round(
                parseInt(
                  "0" + util$f.getValue(lines, "DesignCapacity", "="),
                  10,
                ) * (result2.voltage || 1),
              );
              result2.manufacturer = "Apple";
              result2.serial =
                util$f.getValue(lines, "BatterySerialNumber", "=") ||
                util$f.getValue(lines, "Serial", "=");
              ...
              if (parts && parts[1]) {
                ...
                result2.isCharging =
                  util$f.getValue(lines, "ischarging", "=").toLowerCase() ===
                  "yes";
                result2.acConnected = result2.isCharging;
              }
              if (result2.maxCapacity && result2.currentCapacity) {
                ...
                if (!result2.isCharging) {
                  result2.timeRemaining = parseInt(
                    "0" + util$f.getValue(lines, "TimeRemaining", "="),
                    10,
                  );
                }
              }
      ...
      if (_windows$c) {
        try {
          ...
          util$f.promiseAll(workload).then((data) => {
            if (data) {
...
function graphics(callback) {
      ...
      if (device["CL_DEVICE_TYPE"] === "CL_DEVICE_TYPE_GPU") {
        ...
        if (busAddress) {
          let controller = controllers.find(
            (controller2) => controller2.busAddress === busAddress,
          );
          if (!controller) {
            controller = {
              vendor: "",
  ...
  function getNvidiaSmi() {
    ...
    if (_windows$b) {
      try {
        const basePath =
          util$e.WINDIR + "\\System32\\DriverStore\\FileRepository";
        ...
        const targetDir = candidateDirs.reduce((prevDir, currentDir) => {
          const previousNvidiaSmi = fs$n.statSync(
            [basePath, prevDir, "nvidia-smi.exe"].join("/"),
          );
          const currentNvidiaSmi = fs$n.statSync(
  ...
  function nvidiaSmi(options) {
    ...
    if (nvidiaSmiExe) {
      ...
      const cmd =
        nvidiaSmiExe + " " + nvidiaSmiOpts + (_linux$a ? "  2>/dev/null" : "");
      if (_linux$a) {
  ...
  function nvidiaDevices() {
    ...
    let results = gpus.map((gpu) => {
      const splittedData = gpu
        .split(", ")
        .map((value) => (value.includes("N/A") ? void 0 : value));
    ...
    process.nextTick(() => {
      ...
      if (_linux$a) {
        ...
        exec$a(cmd, function (error, stdout) {
          ...
          exec$a(cmd2, function (error2, stdout2) {
            if (!error2) {
              let lines = stdout2.toString().split("\n");
              result2.controllers = parseLinesLinuxClinfo(
                result2.controllers,
                lines,
              );
            }
      ...
      if (_windows$b) {
        try {
          const workload = [];
          workload.push(
            util$e.powerShell("Get-CimInstance win32_VideoController | fl *"),
          );
          workload.push(
            util$e.powerShell(
          ...
          workload.push(
            util$e.powerShell("Get-CimInstance win32_desktopmonitor | fl *"),
          );
          workload.push(
            util$e.powerShell(
          ...
          Promise.all(workload)
            .then((data) => {
              ...
              result2.controllers = parseLinesWindowsControllers(
                csections,
                vsections,
              );
              result2.controllers = result2.controllers.map((controller) => {
              ...
              result2.displays = parseLinesWindowsDisplaysPowershell(
                ssections,
                msections,
                dsections,
                tsections,
        ...
        if (vections[i].trim() !== "") {
          ...
          if (matchingDeviceId) {
            const quadWordmemorySize = parseInt(
              util$e.getValue(lines, "HardwareInformation.qwMemorySize"),
            );
      ...
      if ({}.hasOwnProperty.call(sections, i)) {
        if (sections[i].trim() !== "") {
          ...
          controllers.push({
            ...
            bus: util$e.getValue(lines, "PNPDeviceID", ":").startsWith("PCI")
              ? "PCI"
              : "",
            vram:
              (memorySize == null
                ? util$e.toInt(util$e.getValue(lines, "AdapterRAM", ":"))
                : memorySize) /
              1024 /
              1024,
          ...
          _resolutionX =
            util$e.toInt(
              util$e.getValue(lines, "CurrentHorizontalResolution", ":"),
            ) || _resolutionX;
          _resolutionY =
            util$e.toInt(
              util$e.getValue(lines, "CurrentVerticalResolution", ":"),
            ) || _resolutionY;
          _refreshRate =
            util$e.toInt(util$e.getValue(lines, "CurrentRefreshRate", ":")) ||
            _refreshRate;
          _pixelDepth =
            util$e.toInt(util$e.getValue(lines, "CurrentBitsPerPixel", ":")) ||
            _pixelDepth;
        }
      }
    }
  ...
  function parseLinesWindowsDisplaysPowershell(
    ...
    if (dsections && dsections.length) {
      ...
      deviceID = util$e
        ...
        .replace(/&amp;/g, "&")
        .toLowerCase();
      resolutionX = util$e.toInt(
        util$e.getValue(linesDisplay, "ScreenWidth", ":"),
      );
      resolutionY = util$e.toInt(
        util$e.getValue(linesDisplay, "ScreenHeight", ":"),
      );
    }
    for (let i = 0; i < ssections.length; i++) {
      if (ssections[i].trim() !== "") {
        ...
        const instanceName = util$e
          .getValue(linesMonitor, "InstanceName")
          .toLowerCase();
        const videoOutputTechnology = util$e.getValue(
          linesConnection,
      ...
      if (line !== "") {
        ...
        if (
          ...
          (line[0].startsWith("/") ||
            ...
            line[0].indexOf("/") > 0 ||
            line[0].indexOf(":") === 1 ||
            (!_darwin$9 && !isLinuxTmpFs(line[1])))
        ) {
          const fs2 = line[0];
          const fsType =
            _linux$9 || _freebsd$8 || _openbsd$8 || _netbsd$8
              ? line[1]
          ...
          const use = parseFloat(
            (100 * (used / (used + available))).toFixed(2),
          );
          let rw =
            osMounts && Object.keys(osMounts).length > 0
              ? osMounts[fs2] || false
              : null;
          line.splice(
            0,
            _linux$9 || _freebsd$8 || _openbsd$8 || _netbsd$8 ? 6 : 5,
          );
    ...
    process.nextTick(() => {
      ...
      if (_linux$9 || _freebsd$8 || _openbsd$8 || _netbsd$8 || _darwin$9) {
        ...
        if (_darwin$9) {
          cmd = "df -kP";
          try {
            macOsDisks = execSync$5("diskutil list")
              .toString()
              .split("\n")
              .filter((line) => {
            ...
            execSync$5("mount")
              ...
              .forEach((line) => {
                osMounts[line.split(" ")[0]] =
                  line.toLowerCase().indexOf("read-only") === -1;
              });
          } catch (e) {
            util$d.noop();
          }
        ...
        if (_linux$9) {
          try {
            ...
            execSync$5("cat /proc/mounts 2>/dev/null", util$d.execOptsLinux)
              .toString()
              .split("\n")
              .filter((line) => {
                return line.startsWith("/");
          ...
          util$d.powerShell(cmd).then((stdout, error) => {
            if (!error) {
              ...
              devices.forEach(function (device) {
                ...
                const free = util$d.toInt(
                  util$d.getValue(lines, "freespace", ":"),
                );
    ...
    process.nextTick(() => {
      ...
      if (_freebsd$8 || _openbsd$8 || _netbsd$8 || _darwin$9) {
        ...
        exec$9(cmd, { maxBuffer: 1024 * 1024 }, function (error, stdout) {
          if (!error) {
            let lines = stdout.toString().split("\n");
            result2.max = parseInt(
              util$d.getValue(lines, "kern.maxfiles", ":"),
              10,
            );
            result2.allocated =
              parseInt(util$d.getValue(lines, "kern.num_files", ":"), 10) ||
              parseInt(util$d.getValue(lines, "kern.open_files", ":"), 10);
      ...
      if (_linux$9) {
        fs$m.readFile("/proc/sys/fs/file-nr", function (error, stdout) {
          if (!error) {
            let lines = stdout.toString().split("\n");
            if (lines[0]) {
            ...
            fs$m.readFile("/proc/sys/fs/file-max", function (error2, stdout2) {
              if (!error2) {
                let lines = stdout2.toString().split("\n");
                if (lines[0]) {
...
function decodeMdabmData(lines) {
  ...
  lines.forEach((line) => {
    if (
      line.toLowerCase().startsWith("md_device_dev") &&
      line.toLowerCase().indexOf("/dev/") > 0
    ) {
  ...
  try {
    data.forEach((element) => {
      if (element.type.startsWith("raid")) {
        const lines = execSync$5(
          `mdadm --export --detail /dev/${element.name}`,
          util$d.execOptsLinux,
        )
          .toString()
          .split("\n");
        ...
        if (
          mdData.members &&
          mdData.members.length &&
          mdData.raid === element.type
        ) {
    ...
    process.nextTick(() => {
      ...
      if (_windows$a) {
        ...
        try {
          ...
          util$d.promiseAll(workload).then((res) => {
...
function calcDiskIO(rIO, wIO, rWaitTime, wWaitTime, tWaitTime) {
  ...
  if (_disk_io && _disk_io.ms) {
    ...
    result2.rWaitPercent =
      ((result2.rWaitTime - _disk_io.rWaitTime) * 100) / result2.ms;
    result2.wWaitPercent =
      ((result2.wWaitTime - _disk_io.wWaitTime) * 100) / result2.ms;
    result2.tWaitPercent =
      ((result2.tWaitTime - _disk_io.tWaitTime) * 100) / result2.ms;
    _disk_io.rIO = rIO;
    _disk_io.wIO = wIO;
              ...
              try {
                ...
                devices.forEach((device) => {
                  ...
                  result2.push({
                    ...
                    name: device.model || "",
                    vendor:
                      getVendorFromModel2(device.model) ||
                    ...
                    data.forEach((disk) => {
                      ...
                      for (let i = 0; i < result2.length; i++) {
                        if (result2[i].BSDName === diskBSDName) {
                          result2[i].smartStatus = disk.smart_status.passed
                            ? "Ok"
            ...
            if (!error) {
              ...
              try {
                ...
                devices.forEach(function (device) {
                  ...
                  const mediumType = util$d
                    .getValue(lines2, "Medium Type", ":", true)
                    .trim();
                  const sizeStr = util$d
                    .getValue(lines2, "capacity", ":", true)
                    .trim();
                  const BSDName = util$d
                    .getValue(lines2, "BSD Name", ":", true)
                    .trim();
                  if (sizeStr) {
                    let sizeValue = 0;
                    ...
                    if (sizeValue) {
                      const smartStatusString = util$d
                        .getValue(lines2, "S.M.A.R.T. status", ":", true)
                        .trim()
                        .toLowerCase();
                      result2.push({
                        device: BSDName,
                        ...
                        name: util$d
                          .getValue(lines2, "Model", ":", true)
                          .trim(),
                        vendor:
                          getVendorFromModel2(
                            util$d.getValue(lines2, "Model", ":", true).trim(),
                          ) ||
                          util$d.getValue(lines2, "Manufacturer", ":", true),
                        size: sizeValue,
                        bytesPerSector: null,
                        totalCylinders: null,
                        totalHeads: null,
                        ...
                        tracksPerCylinder: null,
                        sectorsPerTrack: null,
                        firmwareRevision: util$d
                          .getValue(lines2, "Revision", ":", true)
                          .trim(),
                        serialNum: util$d
                          .getValue(lines2, "Serial Number", ":", true)
                          .trim(),
                        interfaceType: util$d
                          .getValue(lines2, "InterfaceType", ":", true)
                          .trim(),
                        smartStatus:
              ...
              try {
                ...
                devices.forEach(function (device) {
                  ...
                  let lines2 = device.split("\n");
                  const linkWidth = util$d
                    .getValue(lines2, "link width", ":", true)
                    .trim();
                  const sizeStr = util$d
                    .getValue(lines2, "!capacity", ":", true)
                    .trim();
      ...
      if (_windows$a) {
        try {
          ...
          if (util$d.smartMonToolsInstalled()) {
            try {
              const smartDev = JSON.parse(
                execSync$5("smartctl --scan -j").toString(),
              );
          ...
          util$d.promiseAll(workload).then((data) => {
            ...
            devices.forEach(function (device) {
              ...
              if (size) {
                result2.push({
                  ...
                  vendor: getVendorFromModel2(
                    util$d.getValue(lines, "Caption", ":", true).trim(),
                  ),
                  size: parseInt(size),
                  bytesPerSector: parseInt(
                    util$d.getValue(lines, "BytesPerSector", ":"),
                  ),
                  totalCylinders: parseInt(
                    util$d.getValue(lines, "TotalCylinders", ":"),
                  ),
                  totalHeads: parseInt(
                    util$d.getValue(lines, "TotalHeads", ":"),
                  ),
                  totalSectors: parseInt(
                    util$d.getValue(lines, "TotalSectors", ":"),
                  ),
                  totalTracks: parseInt(
                    util$d.getValue(lines, "TotalTracks", ":"),
                  ),
                  tracksPerCylinder: parseInt(
                    util$d.getValue(lines, "TracksPerCylinder", ":"),
                  ),
                  sectorsPerTrack: parseInt(
                    util$d.getValue(lines, "SectorsPerTrack", ":"),
                  ),
                  firmwareRevision: util$d
            ...
            devices.forEach(function (device) {
              ...
              const interfaceType = util$d
                .getValue(lines, "BusType", ":")
                .trim();
...
function getMacAddresses() {
  ...
  if (_linux$8 || _freebsd$7 || _openbsd$7 || _netbsd$7) {
    if (typeof pathToIp === "undefined") {
      try {
        const lines = execSync$4("which ip", util$c.execOptsLinux)
          .toString()
          .split("\n");
        if (
          lines.length &&
    ...
    try {
      if ({}.hasOwnProperty.call(sections, i)) {
        if (sections[i].trim() !== "") {
          ...
          let adapterType =
            util$c.getValue(lines, "AdapterTypeID", ":") === "9"
              ? "wireless"
              : "wired";
          let ifacename = util$c
            .getValue(lines, "Name", ":")
            .replace(/\]/g, ")")
            .replace(/\[/g, "(");
          let iface = util$c
          ...
          if (netEnabled !== "") {
            const speed =
              parseInt(util$c.getValue(lines, "speed", ":").trim(), 10) / 1e6;
            nics.push({
              mac: util$c.getValue(lines, "MACAddress", ":").toLowerCase(),
              dhcp:
                util$c
                  .getValue(linesNicConfig, "dhcpEnabled", ":")
                  .toLowerCase() === "true",
              name: ifacename,
              iface,
              ...
              operstate:
                util$c.getValue(lines, "NetConnectionStatus", ":") === "2"
                  ? "up"
                  : "down",
              type: adapterType,
            });
...
function getWindowsNics() {
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      cmd +=
        "Get-CimInstance Win32_NetworkAdapterConfiguration | fl DHCPEnabled";
      try {
...
function getWindowsIEEE8021x(connectionType, iface, ifaces) {
  ...
  if (connectionType == "wired" && ifaces.length > 0) {
    ...
    try {
      ...
      if (SSID !== "Unknown") {
        let ifaceSanitized = "";
        const s = util$c.isPrototypePolluted()
          ? "---"
          : util$c.sanitizeShellString(SSID);
...
function parseLinesDarwinNics(sections) {
  ...
  sections.forEach((section) => {
    ...
    nic.type =
      util$c.getValue(section, "type").toLowerCase().indexOf("wi-fi") > -1
        ? "wireless"
        : "wired";
    ...
    nic.operstate =
      operstate === "active"
        ...
        : operstate === "inactive"
          ...
          : "unknown";
    nic.duplex =
      util$c.getValue(section, "media").toLowerCase().indexOf("half-duplex") >
      -1
        ? "half"
        : "full";
...
function getDarwinNics() {
  ...
  try {
    const lines = execSync$4(cmd, { maxBuffer: 1024 * 2e4 })
      .toString()
      .split("\n");
    ...
    return (
      virtualMacs.filter((item) => {
        return mac
          .toUpperCase()
          .toUpperCase()
          .startsWith(item.substring(0, mac.length));
      }).length > 0 ||
      ...
      if (_darwin$8 || _freebsd$7 || _openbsd$7 || _netbsd$7) {
        if (JSON.stringify(ifaces) === JSON.stringify(_ifaces) && !rescan) {
          ...
          nics.forEach((nic) => {
            ...
            const s = util$c.isPrototypePolluted()
              ? "---"
              : util$c.sanitizeShellString(nic.iface);
        ...
        if (JSON.stringify(ifaces) === JSON.stringify(_ifaces) && !rescan) {
          ...
          for (let dev in ifaces) {
            ...
            if ({}.hasOwnProperty.call(ifaces, dev)) {
              ...
              const s = util$c.isPrototypePolluted()
                ? "---"
                : util$c.sanitizeShellString(iface);
              const l = util$c.mathMin(s.length, 2e3);
              for (let i = 0; i <= l; i++) {
                if (s[i] !== void 0) {
                  ifaceSanitized = ifaceSanitized + s[i];
            ...
            echo -n "wirelessspeed: "; iw dev ${ifaceSanitized} link 2>&1 | grep bitrate; echo;`;
              let lines = [];
              try {
                lines = execSync$4(cmd, util$c.execOptsLinux)
                  .toString()
                  .split("\n");
                const connectionName =
                  getLinuxIfaceConnectionName(ifaceSanitized);
              ...
              let wirelessspeed = util$c
                .getValue(lines, "wirelessspeed")
                .split("tx bitrate: ");
              if (speed === null && wirelessspeed.length === 2) {
                myspeed = parseFloat(wirelessspeed[1]);
                speed = isNaN(myspeed) ? null : myspeed;
              }
              carrierChanges = parseInt(
                util$c.getValue(lines, "carrier_changes"),
                10,
              );
              const operstate = util$c.getValue(lines, "operstate");
              type =
                operstate === "up"
                  ? util$c.getValue(lines, "wireless").trim()
                    ? "wireless"
                    : "wired"
                  : "unknown";
              if (
                ifaceSanitized === "lo" ||
                ifaceSanitized.startsWith("bond")
              ) {
                type = "virtual";
              }
              let internal =
        ...
        if (JSON.stringify(ifaces) === JSON.stringify(_ifaces) && !rescan) {
          ...
          getWindowsNics().then(function (nics2) {
            ...
            for (let dev in ifaces) {
              ...
              const s = util$c.isPrototypePolluted()
                ? "---"
                : util$c.sanitizeShellString(dev);
              const l = util$c.mathMin(s.length, 2e3);
              for (let i = 0; i <= l; i++) {
                if (s[i] !== void 0) {
                  ifaceSanitized = ifaceSanitized + s[i];
              ...
              if ({}.hasOwnProperty.call(ifaces, dev)) {
                ...
                dnsSuffix = getWindowsIfaceDNSsuffix(
                  dnsSuffixes.ifaces,
                  ifaceSanitized,
                );
                ...
                const IEEE8021x = getWindowsIEEE8021x(
                  type,
                  ifaceSanitized,
                  nics8021xInfo,
                );
  ...
  if (_network[iface] && _network[iface].ms) {
    ...
    result2.rx_sec =
      rx_bytes - _network[iface].rx_bytes >= 0
        ? (rx_bytes - _network[iface].rx_bytes) / (result2.ms / 1e3)
        : 0;
    result2.tx_sec =
      tx_bytes - _network[iface].tx_bytes >= 0
        ? (tx_bytes - _network[iface].tx_bytes) / (result2.ms / 1e3)
        : 0;
        ...
        if (sections[i].trim() !== "") {
          ...
          perfData.push({
            name: util$c
              ...
              .toLowerCase(),
            rx_bytes: parseInt(
              util$c.getValue(lines, "BytesReceivedPersec", ":"),
              10,
            ),
            rx_errors: parseInt(
              util$c.getValue(lines, "PacketsReceivedErrors", ":"),
              10,
            ),
            rx_dropped: parseInt(
              util$c.getValue(lines, "PacketsReceivedDiscarded", ":"),
              10,
            ),
            tx_bytes: parseInt(
              util$c.getValue(lines, "BytesSentPersec", ":"),
              10,
            ),
            tx_errors: parseInt(
              util$c.getValue(lines, "PacketsOutboundErrors", ":"),
              10,
            ),
            tx_dropped: parseInt(
              util$c.getValue(lines, "PacketsOutboundDiscarded", ":"),
              10,
            ),
          });
        }
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (
        ...
        if (_linux$8) {
          if (fs$l.existsSync("/sys/class/net/" + ifaceSanitized)) {
            cmd =
              "cat /sys/class/net/" +
              ifaceSanitized +
        ...
        if (_darwin$8) {
          ...
          exec$8(cmd, function (error, stdout) {
            ...
            result2.operstate =
              result2.operstate === "active"
                ? "up"
                : result2.operstate === "inactive"
                  ? "down"
                  : "unknown";
...
function ifaceListLinux() {
  ...
  try {
    const all = execSync$3(cmd, util$b.execOptsLinux)
      .toString()
      .split("\n")
      .map((line) => line.trim())
      .join("\n");
...
function getWifiNetworkListNmi() {
  ...
  try {
    ...
    parts.forEach((part) => {
      ...
      const frequency = util$b
        .getValue(lines, "FREQ")
        .toLowerCase()
        .replace("mhz", "")
        .trim();
      const security = util$b
        .getValue(lines, "SECURITY")
        .replace("(", "")
        .replace(")", "");
      const wpaFlags = util$b
        .getValue(lines, "WPA-FLAGS")
        .replace("(", "")
        .replace(")", "");
      const rsnFlags = util$b
    ...
    if (iwlistParts.length > 1) {
      ...
      iwlistParts.forEach((element) => {
        ...
        result2.push({
          ...
          frequency: frequency
            ? util$b.toInt(frequency.replace(".", ""))
            : null,
          signalLevel: db,
          quality,
          security,
...
function parseWifiDarwin(wifiStr) {
  ...
  try {
    ...
    wifiObj.forEach(function (wifiItem) {
      ...
      const channel =
        parseInt(("" + wifiItem.spairport_network_channel).split(" ")[0]) || 0;
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux$7) {
        ...
        if (result2.length === 0) {
          try {
            const iwconfigParts = execSync$3(
            ...
            if (iface) {
              let ifaceSanitized = "";
              const s = util$b.isPrototypePolluted()
                ? "---"
                : util$b.sanitizeShellString(iface, true);
    ...
    process.nextTick(() => {
      ...
      if (_linux$7) {
        ...
        ifaces.forEach((ifaceDetail) => {
          ...
          const s = util$b.isPrototypePolluted()
            ? "---"
            : util$b.sanitizeShellString(ifaceDetail.iface, true);
          ...
          let ssidSanitized = "";
          const t = util$b.isPrototypePolluted()
            ? "---"
            : util$b.sanitizeShellString(ssid, true);
          ...
          const channel =
            network2 && network2.length && network2[0].channel
              ? network2[0].channel
              : wpaDetails.channel
                ? wpaDetails.channel
                : null;
          const bssid =
            network2 && network2.length && network2[0].bssid
              ? network2[0].bssid
              : wpaDetails.bssid
                ? wpaDetails.bssid
                : null;
          const signalLevel =
            network2 && network2.length && network2[0].signalLevel
              ? network2[0].signalLevel
              : null;
          if (ssid && bssid) {
            result2.push({
              id: ifaceDetail.id,
        ...
        exec$7(cmd, function (error, stdout) {
          try {
            ...
            const networkObj =
              profilerObj[0]._SPCommandLineArguments.indexOf(
                "SPNetworkDataType",
              ) >= 0
            ...
            const airportWifiObj =
              airportObj[0].spairport_current_network_information;
            const channel =
              parseInt(
                ("" + airportWifiObj.spairport_network_channel).split(" ")[0],
              ) || 0;
            ...
            if (sm === "spairport_security_mode_wep") {
              ...
              sm.startsWith("spairport_security_mode_wpa2_enterprise")
            ) {
              security.push("WPA2 EAP");
            } else if (
              sm.startsWith("pairport_security_mode_wpa3_transition")
            ) {
              security.push("WPA2/WPA3");
            } else if (sm.startsWith("pairport_security_mode_wpa3")) {
              security.push("WPA3");
        ...
        util$b.powerShell(cmd).then(function (stdout) {
          ...
          parts.forEach((part) => {
            ...
            if (lines.length >= 5) {
              ...
              const bssid =
                util$b.getValue(lines, "BSSID", ":", true) ||
                util$b.getValue(lines, "AP BSSID", ":", true);
              const quality = util$b.getValue(lines, "Signal", ":", true);
              const signalLevel = wifiDBFromQuality(quality);
              const type =
                util$b.getValue(lines, "Radio type", ":", true) ||
                util$b.getValue(lines, "Type de radio", ":", true) ||
                util$b.getValue(lines, "Funktyp", ":", true) ||
                null;
              const security =
                util$b.getValue(lines, "authentication", ":", true) ||
                util$b.getValue(lines, "Authentification", ":", true) ||
                util$b.getValue(lines, "Authentifizierung", ":", true) ||
                null;
              const channel =
                util$b.getValue(lines, "Channel", ":", true) ||
                util$b.getValue(lines, "Canal", ":", true) ||
                util$b.getValue(lines, "Kanal", ":", true) ||
                null;
              const txRate =
                util$b.getValue(lines, "Transmit rate (mbps)", ":", true) ||
                util$b.getValue(lines, "Transmission (mbit/s)", ":", true) ||
                util$b.getValue(lines, "Empfangsrate (MBit/s)", ":", true) ||
                null;
    ...
    process.nextTick(() => {
      ...
      if (srv) {
        ...
        if (_linux$6 || _freebsd$6 || _openbsd$6 || _netbsd$6 || _darwin$6) {
          if (
            (_linux$6 || _freebsd$6 || _openbsd$6 || _netbsd$6) &&
            srvString === "*"
          ) {
            try {
              const tmpsrv = execSync$2(
                "systemctl --all --type=service --no-legend 2> /dev/null",
                util$a.execOptsLinux,
              )
                .toString()
                .split("\n");
              srvs = [];
                    ...
                    ps = lines.filter(function (e) {
                      return (
                        ...
                        e
                          ...
                          .indexOf(
                            " " +
                              srv2.toLowerCase().replace(/[0-9.]/g, "") +
                              ":",
                          ) !== -1 ||
                        e.toLowerCase().indexOf("/" + srv2.toLowerCase()) !== -1
                      );
                    });
                  }
                  const pids = [];
                ...
                serviceSections.forEach((element) => {
                  if (element.trim() !== "") {
                    ...
                    let srvName = util$a
                      ...
                      .toLowerCase();
                    let srvCaption = util$a
                      .getValue(lines, "Caption", ":", true)
                      .toLowerCase();
                    let started = util$a.getValue(lines, "Started", ":", true);
                    let startMode = util$a.getValue(
                      lines,
                      "StartMode",
                      ":",
                      true,
    ...
    function parseElapsed(etime) {
      ...
      if (etime.indexOf("d") >= 0) {
        ...
        started = formatDateTime(
          new Date(
            Date.now() -
              (elapsed_parts[0] * 24 + elapsed_parts[1] * 1) * 60 * 60 * 1e3,
          ),
        );
        ...
        started = formatDateTime(
          new Date(
            Date.now() -
              (elapsed_parts[0] * 60 + elapsed_parts[1] * 1) * 60 * 1e3,
          ),
        );
          ...
          new Date(
            Date.now() -
              (elapsed_parts.length > 1
                ? (elapsed_parts[0] * 60 + elapsed_parts[1]) * 1e3
                : elapsed_parts[0] * 1e3),
          ),
        );
      }
    ...
    lines.forEach(function (line) {
      if (line.trim() !== "") {
        ...
        const pmem = parseFloat(
          ((1 * parseInt(parts[3]) * 1024) / os$1.totalmem()).toFixed(1),
        );
                ...
                exec$6(
                  ...
                  function (error2, stdout2) {
                    ...
                    curr_processes.forEach((element) => {
                      ...
                      if (resultProcess.pid) {
                        let listPos = result2.list
                          .map(function (e) {
                            return e.pid;
                          })
                          .indexOf(resultProcess.pid);
                        if (listPos >= 0) {
                          result2.list[listPos].cpu =
                            resultProcess.cpuu + resultProcess.cpus;
                ...
                if (!error) {
                  ...
                  processSections.forEach((element) => {
                    if (element.trim() !== "") {
                      let lines = element.trim().split("\r\n");
                      let pid = parseInt(
                        util$a.getValue(lines, "ProcessId", ":", true),
                        10,
                      );
                      let parentPid = parseInt(
                        util$a.getValue(lines, "ParentProcessId", ":", true),
                        10,
                      );
                      let statusValue = util$a.getValue(
                        lines,
                        "ExecutionState",
                        ":",
                      );
                      let name = util$a.getValue(lines, "Caption", ":", true);
                      let commandLine = util$a.getValue(
                        lines,
                        "CommandLine",
                        ":",
                        true,
                      ...
                      let commandPath = util$a.getValue(
                        lines,
                        "ExecutablePath",
                        ":",
                        true,
                      ...
                      let utime = parseInt(
                        util$a.getValue(lines, "UserModeTime", ":", true),
                        10,
                      );
                      let stime = parseInt(
                        util$a.getValue(lines, "KernelModeTime", ":", true),
                        10,
                      );
                      let memw = parseInt(
                        util$a.getValue(lines, "WorkingSetSize", ":", true),
                        10,
                      );
                      ...
                      procs.push({
                        ...
                        priority: parseInt(
                          util$a.getValue(lines, "Priority", ":", true),
                          10,
                        ),
                        memVsz: parseInt(
                          util$a.getValue(lines, "PageFileUsage", ":", true),
                          10,
                        ),
                        memRss: Math.floor(
                          parseInt(
...
function processLoad(proc, callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      let result2 = [];
      const procSanitized = util$a.isPrototypePolluted()
        ? ""
        : util$a.sanitizeShellString(proc) || "*";
                ...
                if (!error) {
                  ...
                  processSections.forEach((element) => {
                    if (element.trim() !== "") {
                      ...
                      processes2.forEach(function (proc2) {
                        if (
                          name.toLowerCase().indexOf(proc2.toLowerCase()) >=
                            0 &&
                          !inList
                        ) {
                          inList = true;
                  ...
                  procStats.forEach((element) => {
                    ...
                    for (let j = 0; j < result2.length; j++) {
                      if (
                        result2[j].pid === resultProcess.pid ||
                        result2[j].pids.indexOf(resultProcess.pid) >= 0
                      ) {
                        listPos = j;
                      }
                    }
            ...
            if (stdout) {
              ...
              lines.forEach(function (line) {
                ...
                if (data.length > 4) {
                  const linuxName =
                    data[4].indexOf("/") >= 0
                      ? data[4].substring(0, data[4].indexOf("/"))
                      : data[4];
                  const name = _linux$6
                    ? linuxName
              ...
              procStats.forEach(function (item) {
                ...
                for (let j = 0; j < result2.length; j++) {
                  if (
                    item.name
                      .toLowerCase()
                      .indexOf(result2[j].proc.toLowerCase()) >= 0
                  ) {
                    listPos = j;
                  }
                }
                processes2.forEach(function (proc2) {
                  if (
                    item.name.toLowerCase().indexOf(proc2.toLowerCase()) >= 0 &&
                    !inList
                  ) {
                    inList = true;
                    name = proc2;
      ...
      if (is_whopart) {
        ...
        who_line = result_who.filter(function (obj) {
          return (
            ...
            (obj.tty.substring(3, 1e3) === result_w.tty ||
              obj.tty === result_w.tty)
          );
        });
        if (who_line.length === 1) {
...
function parseWinLoggedOn(loggedonParts) {
  ...
  loggedonParts.forEach((loggedon) => {
    ...
    const name =
      parts.length > 2 ? parts[1].split(",")[0].replace(/"/g, "").trim() : "";
    const domain =
      parts.length > 2
        ? parts[2].replace(/"/g, "").replace(/\)/g, "").trim()
        : "";
...
function inetLatency(host, callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      let hostSanitized = "";
      const s = (
        util$8.isPrototypePolluted()
          ? "8.8.8.8"
          : util$8.sanitizeShellString(host, true)
      ).trim();
      const l = util$8.mathMin(s.length, 2e3);
      for (let i = 0; i <= l; i++) {
        if (!(s[i] === void 0)) {
      ...
      if (_sunos$4) {
        ...
        util$8.execSafe("ping", params2, { timeout: 3e3 }).then((stdout) => {
          ...
          if (stdout) {
            const lines = stdout
              .split("\n")
              .filter((line2) => line2.indexOf(filt) >= 0)
              .join("\n");
            const line = lines.split("=");
            if (line.length > 1) {
              const parts = line[1].split("/");
      ...
      if (_windows$5) {
        ...
        try {
          ...
          util$8
            .execSafe("ping", params2, util$8.execOptsWin)
            .then((stdout) => {
              if (stdout) {
                let lines = stdout.split("\r\n");
                lines.shift();
    ...
    process.nextTick(() => {
      ...
      _docker_socket.listImages(all, (data) => {
        ...
        try {
          ...
          if (
            dockerImages2 &&
            Object.prototype.toString.call(dockerImages2) ===
              "[object Array]" &&
            dockerImages2.length > 0
          ) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      const imageIDSanitized = (
        util$7.isPrototypePolluted()
          ? ""
          : util$7.sanitizeShellString(imageID, true)
      ).trim();
      if (imageIDSanitized) {
        if (!_docker_socket) {
          _docker_socket = new DockerSocket2();
          ...
          try {
            resolve({
              ...
              created: data.Created
                ? Math.round(new Date(data.Created).getTime() / 1e3)
                : 0,
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      const containerIdSanitized = (
        util$7.isPrototypePolluted()
          ? ""
          : util$7.sanitizeShellString(containerID, true)
      ).trim();
            ...
            resolve({
              ...
              started:
                data.State && data.State.StartedAt
                  ? Math.round(new Date(data.State.StartedAt).getTime() / 1e3)
                  : 0,
              finished:
                data.State &&
      ...
      if (util$7.isFunction(containerIDs) && !callback) {
        ...
        if (containerIDsSanitized !== "*") {
          ...
          const s = (
            util$7.isPrototypePolluted()
              ? ""
              : util$7.sanitizeShellString(containerIDs, true)
          ).trim();
                ...
                if (!stats.message) {
                  ...
                  result2.pids =
                    stats.pids_stats && stats.pids_stats.current
                      ...
                      : 0;
                  result2.restartCount = dataInspect.RestartCount
                  ...
                  result2.precpuStats = stats.precpu_stats
                    ? stats.precpu_stats
                    : {};
                  result2.memoryStats = stats.memory_stats
                    ? stats.memory_stats
                    : {};
    ...
    process.nextTick(() => {
      ...
      _docker_socket.listVolumes((data) => {
        ...
        try {
          ...
          if (
            dockerVolumes2 &&
            dockerVolumes2.Volumes &&
            Object.prototype.toString.call(dockerVolumes2.Volumes) ===
              "[object Array]" &&
            dockerVolumes2.Volumes.length > 0
          ) {
            dockerVolumes2.Volumes.forEach(function (element) {
              result2.push({
                ...
                created: element.CreatedAt
                  ? Math.round(new Date(element.CreatedAt).getTime() / 1e3)
                  : 0,
              });
            });
            if (callback) {
...
function vboxInfo(callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      try {
        exec$4(
          util$6.getVboxmanage() + " list vms --long",
          function (error, stdout) {
            ...
            parts.forEach((part) => {
              ...
              try {
                if (running) {
                  ...
                  runningSince =
                    Math.round((Date.now() - Date.parse(sinceDateObj)) / 1e3) +
                    offset * 60;
                }
              } catch (e) {
                util$6.noop();
              ...
              result2.push({
                ...
                memory: parseInt(
                  util$6.getValue(lines, "Memory size", "     "),
                  10,
                ),
                ...
                tripleFaultReset:
                  util$6.getValue(lines, "Triple Fault Reset") === "enabled",
...
function parseWindowsPrinters(lines, id) {
  ...
  result2.default =
    util$5.getValue(lines, "Default", ":").toUpperCase() === "TRUE";
  result2.shared =
    util$5.getValue(lines, "Shared", ":").toUpperCase() === "TRUE";
  return result2;
}
            ...
            try {
              ...
              if (
                ...
                for (let i = 0; i < outObj.SPPrintersDataType.length; i++) {
                  const printer2 = parseDarwinPrinters(
...
function parseLinuxUsb(usb2) {
  ...
  const interfaceClass = util$4
    .getValue(lines, "bInterfaceClass", " ", true)
    .trim();
  let interfaceClassParts = interfaceClass.split(" ");
  interfaceClassParts.shift();
  const usbType = interfaceClassParts.join(" ");
  const iManufacturer = util$4
    .getValue(lines, "iManufacturer", " ", true)
    .trim();
...
function parseDarwinUsb(usb2, id) {
  ...
  try {
    ...
    result2.removable = usbObj["non-removable"]
      ? usbObj["non-removable"].toLowerCase() || false
      : true;
    result2.vendor =
...
function parseWindowsUsb(lines, id) {
  const usbType = getWindowsUsbTypeCreation(
    util$4.getValue(lines, "CreationClassName", ":").toLowerCase(),
    util$4.getValue(lines, "name", ":").toLowerCase(),
  );
  if (usbType) {
    const result2 = {};
    result2.bus = null;
          ...
          .then((stdout, error) => {
            if (!error) {
              ...
              for (let i = 0; i < parts.length; i++) {
                ...
                if (
                  usb2 &&
                  result2.filter((x) => x.deviceId === usb2.deviceId).length ===
                    0
                ) {
                  result2.push(usb2);
                }
...
function getLinuxAudioPci() {
  ...
  let result2 = [];
  try {
    const parts = execSync$1(cmd, util$3.execOptsLinux)
      .toString()
      .split("\n\n");
    parts.forEach((element) => {
      ...
      if (
        ...
        audio2.driver =
          util$3.getValue(lines, "Kernel driver in use", ":", true) ||
          util$3.getValue(lines, "Kernel modules", ":", true);
        result2.push(audio2);
      }
    });
    return result2;
...
function parseDarwinBluetoothDevices(bluetoothObject, macAddr2) {
  ...
  result2.manufacturer =
    ...
    "";
  result2.macDevice = (
    bluetoothObject.device_addr ||
    bluetoothObject.device_address ||
    ""
  )
    .toLowerCase()
    .replace(/-/g, ":");
    ...
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        btFiles.forEach((element) => {
          ...
          if (filename === "info") {
            const infoFile = fs$j
              .readFileSync(element, { encoding: "utf8" })
              .split("\n");
            result2.push(parseLinuxBluetoothInfo(infoFile, macAddr1, macAddr2));
          }
        });
        try {
          const hdicon = execSync("hcitool con", util$2.execOptsLinux)
            .toString()
            .toLowerCase();
          for (let i = 0; i < result2.length; i++) {
            if (
              result2[i].macDevice &&
              result2[i].macDevice.length > 10 &&
              hdicon.indexOf(result2[i].macDevice.toLowerCase()) >= 0
            ) {
          ...
          if (!error) {
            try {
              ...
              if (
                ...
                outObj.SPBluetoothDataType[0]["device_title"].forEach(
                  (element) => {
                    const obj = element;
                ...
                const macAddr2 =
                  ...
                  outObj.SPBluetoothDataType[0].controller_properties
                    ...
                    ? outObj.SPBluetoothDataType[0].controller_properties.controller_address
                        ...
                        .replace(/-/g, ":")
                    : null;
                outObj.SPBluetoothDataType[0]["device_connected"].forEach(
                  (element) => {
                    const obj = element;
                    const objKey = Object.keys(obj);
                    if (objKey && objKey.length === 1) {
                ...
                const macAddr2 =
                  ...
                  outObj.SPBluetoothDataType[0].controller_properties
                    ...
                    ? outObj.SPBluetoothDataType[0].controller_properties.controller_address
                        ...
                        .replace(/-/g, ":")
                    : null;
                outObj.SPBluetoothDataType[0]["device_not_connected"].forEach(
                  (element) => {
                    const obj = element;
                    const objKey = Object.keys(obj);
                    if (objKey && objKey.length === 1) {
...
utils$1.checkPath = function checkPath(pth) {
  if (process.platform === "win32") {
    const pathHasInvalidWinCharacters = /[<>:"|?*]/.test(
      pth.replace(path$c.parse(pth).root, ""),
    );
...
async function copy$2(src, dest, opts = {}) {
  ...
  const { srcStat, destStat } = await stat$3.checkPaths(
    src,
    dest,
    "copy",
    opts,
...
const emptyDir = u$8(async function emptyDir2(dir) {
  ...
  return Promise.all(
    items.map((item) => remove$1.remove(path$8.join(dir, item))),
  );
});
...
async function move$1(src, dest, opts = {}) {
  ...
  const { srcStat, isChangingCase = false } = await stat$1.checkPaths(
    src,
    dest,
    "move",
    opts,
...
const aiOptimizationService = {
  ...
  getSuggestions: async () => {
    ...
    try {
      ...
      if (await fs.pathExists(CRASH_LOG_FILE)) {
        const crashes = JSON.parse(await fs.readFile(CRASH_LOG_FILE, "utf-8"));
        if (crashes.length > 5) {
      ...
      if (await fs.pathExists(APP_USAGE_LOG_FILE)) {
        ...
        const appDurations = usageLogs.reduce((acc, log) => {
...
const memoryOptimizerService = {
  ...
  getHistory: async () => {
    try {
      if (await fs.pathExists(MEMORY_LOG_FILE)) {
        const logs = JSON.parse(await fs.readFile(MEMORY_LOG_FILE, "utf-8"));
        return logs;
      }
      return [];
    } catch (error) {
  ...
  createWindow() {
    ...
    if (process$1.env["VITE_DEV_SERVER_URL"]) {
      ...
      this.mainWindow.loadFile(
        path$g.join(app.getAppPath(), "dist", "index.html"),
      );
    }
