...
class AIOptimizationService {
    ...
    try {
      if (await fs.pathExists(OPTIMIZATION_CACHE_FILE)) {
        const cacheData = JSON.parse(
          await fs.readFile(OPTIMIZATION_CACHE_FILE, "utf-8"),
        );
        const cacheAge =
          Date.now() - new Date(cacheData.lastAnalysis).getTime();
  ...
  private calculatePerformanceScore(logs: PerformanceLogEntry[]): number {
    ...
    const avgCpu =
      recent.reduce((sum, log) => sum + log.cpu, 0) / recent.length;
    const avgMem =
      recent.reduce((sum, log) => sum + log.mem, 0) / recent.length;
    const avgDisk =
      recent.reduce((sum, log) => sum + (log.diskUsage || 0), 0) /
      recent.length;
  ...
  private analyzeTrends(
    ...
    const avgRecentCpu =
      recent.reduce((sum, log) => sum + log.cpu, 0) / recent.length;
    const avgOlderCpu =
      older.reduce((sum, log) => sum + log.cpu, 0) / older.length;

    const avgRecentMem =
      recent.reduce((sum, log) => sum + log.mem, 0) / recent.length;
    const avgOlderMem =
      older.reduce((sum, log) => sum + log.mem, 0) / older.length;

    const avgRecentDisk =
      recent.reduce((sum, log) => sum + (log.diskUsage || 0), 0) /
      recent.length;
    const avgOlderDisk =
      older.reduce((sum, log) => sum + (log.diskUsage || 0), 0) / older.length;
    ...
    try {
      ...
      if (await fs.pathExists(PERFORMANCE_LOG_FILE)) {
        ...
        if (trends.cpu === "degrading") {
          suggestions.push(
            "CPU performance is degrading over time. Check for resource-intensive processes.",
          );
        }

        if (trends.memory === "degrading") {
          suggestions.push(
            "Memory usage is increasing over time. Consider restarting applications periodically.",
          );
        }
      ...
      if (await fs.pathExists(CRASH_LOG_FILE)) {
        ...
        const recentCrashes = crashes.filter(
          (crash) =>
            Date.now() - new Date(crash.timestamp).getTime() <
            24 * 60 * 60 * 1000,
        );

        const criticalCrashes = crashes.filter(
          (crash) => crash.severity === "critical",
        ).length;
      ...
      suggestions.push(
        "Unable to analyze system data. Please check system logs for errors.",
      );
    }
