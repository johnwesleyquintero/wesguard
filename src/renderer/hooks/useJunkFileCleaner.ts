...
const useJunkFileCleaner = () => {
  ...
  const categorizeFiles = useCallback((files: JunkFile[]): AnalysisResult["categories"] => {
    const categories = {
      temp: [] as JunkFile[],
      cache: [] as JunkFile[],
      logs: [] as JunkFile[],
  ...
  const generateRecommendations = useCallback((categories: AnalysisResult["categories"]): string[] => {
    const recommendations: string[] = [];
    
    if (categories.temp.length > 50) {
      recommendations.push("Large number of temporary files detected. Regular cleanup recommended.");
    }
    
    if (categories.cache.length > 0) {
      const cacheSize = categories.cache.reduce((sum, file) => sum + file.size, 0);
      if (cacheSize > 100 * 1024 * 1024) { // 100MB
        recommendations.push("Cache files are taking up significant space. Safe to clean.");
      }
    }
    
    if (categories.logs.length > 20) {
      recommendations.push("Multiple log files found. Consider keeping recent logs only.");
    }
    
    if (categories.downloads.length > 0) {
      recommendations.push("Files in Downloads folder detected. Review before cleaning.");
    }
  ...
  const analyze = useCallback(async () => {
    ...
    try {
      ...
      debouncedProgressUpdate(`Analysis completed in ${(analysisTime / 1000).toFixed(1)}s`);
    ...
    try {
      ...
      for (let i = 0; i < filesToDelete.length; i += batchSize) {
        ...
        try {
          ...
          const result = await window.electronAPI.cleaner.executeCleaning(batch);
      ...
      if (deletedCount === filesToDelete.length) {
        ...
        setJunkFiles([]);
        debouncedProgressUpdate(`Successfully cleaned ${deletedCount} files in ${(timeElapsed / 1000).toFixed(1)}s`);
